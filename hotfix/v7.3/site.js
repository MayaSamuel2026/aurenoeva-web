...
Email: ${val(fields.email.value)}`}
 ...
 wizard.querySelector('#email-brief')?.addEventListener('click',async e=>{
  ...
  try{
   ...
   if(sendStatus){sendStatus.className='form-status success';sendStatus.innerHTML=lang==='de'?`Anfrage gesendet. Ihre Referenz: <strong>${escapeHTML(data.reference||'')}</strong>. Die ausgewählten Teilenummern und Produktidentitäten wurden mit der Anfrage übermittelt.`:`Enquiry sent. Your reference: <strong>${escapeHTML(data.reference||'')}</strong>. The selected part numbers and product identities were submitted with your enquiry.`}
